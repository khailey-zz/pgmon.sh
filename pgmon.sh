...
function wts  
{
     ...
     SELECT 
         MAX(CASE WHEN vars.nm = 'AAS' THEN  vars.value  ELSE NULL END) AS "AAS     ",
         MAX(CASE WHEN vars.nm = 'blks_hit' THEN  (select  sum(pg_stat_database.blks_hit) from pg_stat_database) - vars.value  ELSE NULL END) AS "blks_hit",
         MAX(CASE WHEN vars.nm = 'blks_read' THEN  (select  sum(pg_stat_database.blks_read) from pg_stat_database) - vars.value  ELSE NULL END) AS "blks_read",
         MAX(CASE WHEN vars.nm = 'blk_read_time' THEN  (select  sum(pg_stat_database.blk_read_time) from pg_stat_database) - vars.value  ELSE NULL END) AS "blk_read_time",
         MAX(CASE WHEN vars.nm = 'blk_write_time' THEN  (select  sum(pg_stat_database.blk_write_time) from pg_stat_database) - vars.value  ELSE NULL END) AS "blk_write_time",
         MAX(CASE WHEN vars.nm = 'tup_returned' THEN  (select  sum(pg_stat_database.tup_returned) from pg_stat_database) - vars.value  ELSE NULL END) AS "tup_returned",
         MAX(CASE WHEN vars.nm = 'tup_fetched' THEN  (select  sum(pg_stat_database.tup_fetched) from pg_stat_database) - vars.value  ELSE NULL END) AS "tup_fetched",
         MAX(CASE WHEN vars.nm = 'tup_inserted' THEN  (select  sum(pg_stat_database.tup_inserted) from pg_stat_database) - vars.value  ELSE NULL END) AS "tup_inserted",
         MAX(CASE WHEN vars.nm = 'tup_updated' THEN  (select  sum(pg_stat_database.tup_updated) from pg_stat_database) - vars.value  ELSE NULL END) AS "tup_updated",
         MAX(CASE WHEN vars.nm = 'tup_deleted' THEN  (select  sum(pg_stat_database.tup_deleted) from pg_stat_database) - vars.value  ELSE NULL END) AS "tup_deleted",
         MAX(CASE WHEN vars.nm = 'n_tup_del'   THEN  (select  sum(pg_stat_all_tables.n_tup_del) from pg_stat_all_tables) - vars.value  ELSE NULL END) AS "n_tup_del",
         MAX(CASE WHEN vars.nm = 'heap_blks_read'   THEN  (select  sum(pg_statio_all_tables.heap_blks_read) from pg_statio_all_tables) - vars.value  ELSE NULL END) AS "heap_blks_read"
    FROM vars ;
     \o 
     \o /dev/null
     UPDATE vars SET value = ( select sum(pg_stat_database.blks_hit) from pg_stat_database) where nm='blks_hit';
     UPDATE vars SET value = ( select sum(pg_stat_database.blk_write_time) from pg_stat_database) where nm='blk_write_time';
     UPDATE vars SET value = ( select sum(pg_stat_database.blks_read) from pg_stat_database) where nm='blks_read';
     UPDATE vars SET value = ( select sum(pg_stat_database.blk_read_time) from pg_stat_database) where nm='blk_read_time';
     UPDATE vars SET value = ( select sum(pg_stat_database.tup_returned) from pg_stat_database) where nm='tup_returned';
     UPDATE vars SET value = ( select sum(pg_stat_database.tup_fetched) from pg_stat_database) where nm='tup_fetched';
     UPDATE vars SET value = ( select sum(pg_stat_database.tup_inserted) from pg_stat_database) where nm='tup_inserted';
     UPDATE vars SET value = ( select sum(pg_stat_database.tup_updated) from pg_stat_database) where nm='tup_updated';
